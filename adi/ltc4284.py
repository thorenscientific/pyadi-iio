...
class ltc4284(context_manager, attribute):
    ...
    Example channel access::
        ...
        dev.energy1.enable = 1   # Enable energy accumulator
        dev.energy1.input        # Accumulated energy in uJ
    ...
    def __init__(self, uri="", device_index=0):
        ...
        # energy1 has no label in the driver; falls back to ch_id as the key.
    ...
    class _channel_energy(attribute):
        """Energy accumulator channel (energy1).

        No label in the driver — accessible as dev.energy1.
        enable gates the integrator; input is the accumulated value in microjoules (uJ).
        """
        ...
        @property
        def input(self):
            """Accumulated energy in microjoules (uJ)"""
            return self._get_iio_attr(self.name, "input", False)
