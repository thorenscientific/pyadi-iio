...
"""
    ...
    def __post_init__(self):
        ...
        for name in self._rx_channel_names:
            if name == "temp":
                self.channel.append(_temp_channel(self._ctrl, name))
                setattr(self, name.replace("-", "_"), _temp_channel(self._ctrl, name))
            else:
                self.channel.append(_ad7124_channel(self._ctrl, name))
                setattr(self, name.replace("-", "_"), _ad7124_channel(self._ctrl, name))
