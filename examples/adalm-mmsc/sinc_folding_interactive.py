...
parser.add_argument(
    "-m", "--m2k_uri", default=None, help="LibIIO context URI of the ADALM2000"
)
# parser.add_argument('-a', '--ad4080_uri', default='serial:/dev/ttyACM0,230400,8n1',
parser.add_argument(
    "-a",
    "--ad4080_uri",
    default="serial:COM9,230400",
...
class IIOThread(Thread):
    ...
    data_in = np.zeros(
        npts
    )  # Properly scaled waveform, sent from iio_thread to the GUI
    ...
    def run(self):
        ...
        while self.running:
            ...
            if transmitting_center_frequency != self.selected_center_frequency:
                # Generate waveform
                self.status(
                    f"Generating new waveform with center frequency = {self.selected_center_frequency} Hz"
                )
                waveform = workshop.generate_noise_band(
                    self.selected_center_frequency, 10000, fs_out
                )
                print(waveform)
