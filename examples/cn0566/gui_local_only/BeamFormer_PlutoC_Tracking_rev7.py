...
gpios = adi.one_bit_adc_dac('ip:localhost')
...
class App:  
    def __init__(self, master):      
        ...
        self.array = adi.adar1000_array(
            ...
            device_element_map={
                1: [2, 1, 4, 3],          #i.e. channel2 of BEAM0, maps to element 1
                2: [6, 5, 8, 7]           
            }
        )
        ...
        tk.Label(frame1, text="Wait Time (ms)", relief=SUNKEN, anchor=W).grid(row=14, column=2, pady=20, sticky=E+W)
    ...
    def closeProgram(self):
        ...
        time.sleep(1)  # give the program a second to finish any sweep that it is on.  Otherwise you get an error.
    ...
    def programLO(self):
        ...
        if self.Rx_gain != int(self.RxGain.get()):
            ...
            SDR_setRx(self.sdr, self.num_Rx, int(self.Rx_freq), self.Rx_gain)  # also sets Pluto's Tx LO to equal the Rx LO
    ...
    def getData(self, Averages):
        ...
        for count in range (0, Averages):
            ...
            chan1_fft_shift = np.fft.fft(chan1)*np.exp(1.0j*np.deg2rad(self.Beam0_Phase_set.get()))
            chan1 = np.fft.ifft(chan1_fft_shift, n=NumSamples)
            chan1 = chan1[0:NumSamples]
            chan2_fft_shift = np.fft.fft(chan2)*np.exp(1.0j*np.deg2rad(self.Beam1_Phase_set.get()))
    ...
    def plotData(self, plot_gain, plot_fft, plot_tracking):
        ...
        if self.PlotMax_set.get() == 1:
            self.max_gain_line[0].set_data(self.ArrayAngle, np.full(len(self.ArrayAngle),self.peak_gain))
